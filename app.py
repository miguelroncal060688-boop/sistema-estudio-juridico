...
MENU_DEF = [
    ...
    ("Pagos Cuota Litis", "CuotaLitis"),
    ("Cronograma de Cuotas", "CuotaLitis"),
    ("Actuaciones", "Actuaciones"),
    ("Consultas", "Consultas"),
    ("Documentos", "Documentos"),
    ("Plantillas de Contrato", "Documentos"),
    ("Generar Contrato", "Documentos"),
    ("Repositorio Contratos", "Contratos"),
...
if menu == "Colaboradores":
    ...
    # --------------------------------------------------
    # USUARIOS DISPONIBLES (NO ABOGADOS, NO ADMIN)
    # --------------------------------------------------
    ...
    if not usuarios_df.empty and "Usuario" in usuarios_df.columns:
        # normalizar rol y usuario
        if "Rol" not in usuarios_df.columns:
            usuarios_df["Rol"] = ""
        usuarios_df["Rol"] = usuarios_df["Rol"].astype(str).str.strip().str.lower()
        usuarios_df["Usuario"] = usuarios_df["Usuario"].astype(str).str.strip()

        # ✅ EXCLUIR: abogados y admins (incluye "Admin" o "Administrador") y el username "admin"
        usuarios_no_abogado = usuarios_df[
            (usuarios_df["Rol"] != "abogado") &
            (usuarios_df["Rol"] != "admin") &
            (usuarios_df["Rol"] != "administrador") &
            (usuarios_df["Usuario"].str.lower() != "admin")
        ]["Usuario"].tolist()

        # si hay columna Activo, filtrar activos
        if "Activo" in usuarios_df.columns:
            activos = usuarios_df[usuarios_df["Activo"].astype(str) == "1"]["Usuario"].astype(str).str.strip().tolist()
            usuarios_no_abogado = [u for u in usuarios_no_abogado if u in activos]

    # usuarios ya asignados
    usuarios_ya_asignados = colaboradores["Usuario"].astype(str).str.strip().tolist()
    usuarios_ya_asignados = [u for u in usuarios_ya_asignados if u]

    usuarios_disponibles = [u for u in usuarios_no_abogado if u not in usuarios_ya_asignados]
    ...
    if accion == "Nuevo":
        with st.form("colab_new"):
            ...
            if submit:
                ...
                # ✅ VALIDACIÓN EXTRA: impedir admin aunque lo intenten escribir por CSV
                if usuario.strip().lower() == "admin":
                    st.error("No se puede asociar el usuario 'admin' a un colaborador.")
                    st.stop()

                # ✅ impedir roles admin/abogado por seguridad (por si cambian datos)
                if usuario.strip():
                    fila_u = usuarios_df[usuarios_df["Usuario"].astype(str).str.strip() == usuario.strip()]
                    if not fila_u.empty:
                        r = str(fila_u.iloc[0].get("Rol","")).strip().lower()
                        if r in ["admin","administrador","abogado"]:
                            st.error("No se puede asociar un usuario Admin/Abogado como colaborador.")
                            st.stop()
    ...
    # =========================
        ...
        if colaboradores.empty:
            ...
            with st.form("colab_edit"):
                nombre = st.text_input("Nombre completo", value=str(fila.get("Nombre","")))
                dni = st.text_input("DNI", value=str(fila.get("DNI","")))
                tipo = st.selectbox(
                    "Tipo de colaborador",
                    ["Personal Administrativo","Secretaria/o","Practicante"],
                    index=["Personal Administrativo","Secretaria/o","Practicante"].index(str(fila.get("Tipo","Personal Administrativo")))
                    if str(fila.get("Tipo","Personal Administrativo")) in ["Personal Administrativo","Secretaria/o","Practicante"] else 0
                )

                usuario_actual = str(fila.get("Usuario","")).strip()

                # permitir mantener usuario actual aunque ya esté asignado
                opciones_usuario = [""] + [
                    u for u in usuarios_no_abogado
                    if u == usuario_actual or u not in usuarios_ya_asignados
                ]

                # ✅ si por alguna razón usuario_actual es admin, lo forzamos a vacío
                if usuario_actual.lower() == "admin":
                    usuario_actual = ""
                ...
                activo = st.selectbox("Activo", ["1","0"], index=0 if str(fila.get("Activo","1"))=="1" else 1)
                obs = st.text_area("Observaciones", value=str(fila.get("Observaciones","")))
...
# ==========================================================
# HONORARIOS (Total + Por etapa) ✅ DEFINITIVO (EDITAR ROBUSTO)
# - Modo TOTAL vs ETAPAS
# - Edición / Borrado robusto (NO se queda pegado al último)
# - Caso/Bloqueado en edición (se jala del registro)
# ==========================================================
if menu == "Honorarios":
    ...
    exp_list = casos["Expediente"].tolist() if (casos is not None and not casos.empty and "Expediente" in casos.columns) else []
    ...
    with tab_total:
        ...
        if honorarios is None:
            honorarios = pd.DataFrame(columns=["ID","Caso","Monto Pactado","Notas","FechaRegistro"])
        ...
        # -------- EDITAR / BORRAR (ROBUSTO) --------
        st.divider()
        st.markdown("### Editar / borrar honorario total")

        if honorarios is not None and not honorarios.empty:
            ids_total = honorarios["ID"].tolist()
            ...
            # ✅ Form key por ID para evitar “pegado”
            with st.form(f"hon_total_edit_{sel}"):
                # ✅ Caso BLOQUEADO (se jala del registro)
                st.text_input(
                    "Caso (bloqueado)",
                    value=str(fila["Caso"]),
                    disabled=True,
                    key=f"hon_total_caso_view_{sel}"
                )
                ...
                monto_e = st.number_input(
                    ...
                    step=50.0,
                    key=f"hon_total_monto_edit_{sel}"
                )

                notas_e = st.text_input(
                    "Notas",
                    value=str(fila.get("Notas","")),
                    key=f"hon_total_notas_edit_{sel}"
                )
            ...
            if submit:
                ...
                # ✅ NO tocamos Caso. Solo monto y notas.
                honorarios.loc[idx, ["Monto Pactado","Notas"]] = [float(monto_e), notas_e]
                save_df("honorarios", honorarios)
                st.success("✅ Actualizado")
                st.rerun()
            ...
            confirm = st.text_input(
                ...
                key=f"hon_total_del_confirm_{sel}"
            )
            if st.button(
                "🗑️ Borrar",
                disabled=is_readonly or confirm.strip().upper() != "ELIMINAR",
                key=f"hon_total_del_btn_{sel}"
            ):
    ...
    with tab_etapas:
        ...
        if honorarios_etapas is None:
            honorarios_etapas = load_df("honorarios_etapas")
        if honorarios_etapas is None:
            honorarios_etapas = pd.DataFrame(columns=["ID","Caso","Etapa","Monto Pactado","Notas","FechaRegistro"])
        ...
        if honorarios_etapas is not None and not honorarios_etapas.empty:
            ids_et = honorarios_etapas["ID"].tolist()
            ...
            # ✅ Form key por ID para evitar “pegado”
            with st.form(f"hon_et_edit_{sel}"):
                # ✅ Caso BLOQUEADO
                st.text_input(
                    "Caso (bloqueado)",
                    value=str(fila["Caso"]),
                    disabled=True,
                    key=f"hon_et_caso_view_{sel}"
                )
                ...
                monto_e = st.number_input(
                    ...
                    value=_num(fila["Monto Pactado"]),
                    step=50.0,
                    key=f"hon_et_monto_edit_{sel}"
                )

                notas_e = st.text_input(
                    "Notas",
                    value=str(fila.get("Notas","")),
                    key=f"hon_et_notas_edit_{sel}"
                )
            ...
            if submit:
                ...
                # ✅ NO tocamos Caso. Solo etapa/monto/notas.
                honorarios_etapas.loc[idx, ["Etapa","Monto Pactado","Notas"]] = [etapa_e, float(monto_e), notas_e]
                save_df("honorarios_etapas", honorarios_etapas)
                st.success("✅ Actualizado")
                st.rerun()
            ...
            confirm = st.text_input(
                "Escribe ELIMINAR para confirmar",
                key=f"hon_et_del_confirm_{sel}"
            )
            if st.button(
                "🗑️ Borrar",
                disabled=is_readonly or confirm.strip().upper() != "ELIMINAR",
                key=f"hon_et_del_btn_{sel}"
            ):
...
# ==========================================================
# USUARIOS + ROLES + PERMISOS (FASE 1 UI)
# - CRUD usuarios (con vínculo abogado)
# - Permisos por ROL y por USUARIO (Scope/ScopeID)
# - Admin protegido
# ==========================================================
if menu == "Usuarios":
    ...
    # --------------------------------------------------
    # ROLES base (puedes ampliar luego)
    # --------------------------------------------------
    ROLES_DISPONIBLES = [
        "Admin",
        "Abogado",
    ...
    # PERMISOS (FASE 1)
    # =========================
    # Nuevo esquema esperado:
    # SCHEMAS["permisos"] incluye Scope, ScopeID, Ver, Agregar, Modificar, Borrar, ... módulos + dashboard sections
    permisos = load_df("permisos")
    if permisos is None:
        permisos = pd.DataFrame(columns=SCHEMAS.get("permisos", []))

    # --------------------------------------------------
    # MIGRACIÓN automática si permisos.csv aún está en formato viejo:
    # columnas antiguas: Rol, Ver, Agregar, Modificar, Borrar
    # --------------------------------------------------
    def _perm_cols():
        return SCHEMAS.get("permisos", [])

    def _perm_has_new_schema(df: pd.DataFrame) -> bool:
        return (df is not None) and (not df.empty) and ("Scope" in df.columns) and ("ScopeID" in df.columns)
    ...
    # --------------------------------------------------
    def _perm_get(scope: str, scope_id: str) -> pd.Series | None:
        dfp = permisos.copy()
        if dfp is None or dfp.empty:
            return None
        if "Scope" not in dfp.columns or "ScopeID" not in dfp.columns:
            return None
        row = dfp[(dfp["Scope"].astype(str) == scope) & (dfp["ScopeID"].astype(str) == scope_id)]
        return None if row.empty else row.iloc[0]

    def _perm_bool(v) -> bool:
        try:
            return str(v).strip() == "1" or int(v) == 1
        except Exception:
            return str(v).strip().lower() in ["true","yes","si","sí"]

    def can_action(action_col: str) -> bool:
        # action_col: Ver/Agregar/Modificar/Borrar o módulos/dashboard
        usuario_actual = str(st.session_state.get("usuario","")).strip()
        rol_actual = str(st.session_state.get("rol","")).strip()

        # 1) user override
        r_user = _perm_get("USER", usuario_actual)
        if r_user is not None and action_col in r_user.index:
            return _perm_bool(r_user.get(action_col, "0"))

        # 2) role
        r_role = _perm_get("ROLE", rol_actual)
        if r_role is not None and action_col in r_role.index:
            return _perm_bool(r_role.get(action_col, "0"))

        # fallback conservador
        return False

    globals()["has_perm"] = lambda acc: can_action({"ver":"Ver","agregar":"Agregar","modificar":"Modificar","borrar":"Borrar"}.get(acc, "Ver"))

    # --------------------------------------------------
    # UI principal: Tabs Usuarios / Permisos
    # --------------------------------------------------
    tab_usr, tab_perm = st.tabs(["👤 Gestión de usuarios", "🔐 Permisos (Rol / Usuario)"])

    # ==========================================================
    # TAB: PERMISOS (ADMIN)
    # ==========================================================
    with tab_perm:
        st.markdown("### 🔐 Panel de permisos (FASE 1)")
        if not _is_admin():
            st.info("Solo Admin puede editar permisos. (Los demás pueden ver sus accesos pero no cambiar).")

        # lista de columnas de permiso (excluye identity)
        perm_cols = [c for c in SCHEMAS["permisos"] if c not in ["Scope","ScopeID"]]

        tab_r, tab_u = st.tabs(["Por Rol", "Por Usuario"])

        # ---- POR ROL ----
        with tab_r:
            rol_sel = st.selectbox("Rol", ROLES_DISPONIBLES, key="perm_role_sel")
            base_row = _perm_get("ROLE", rol_sel)
            base = {c: "0" for c in perm_cols}
            if base_row is not None:
                for c in perm_cols:
                    base[c] = str(base_row.get(c, "0"))

            # agrupar visualmente
            grp_acc = ["Ver","Agregar","Modificar","Borrar"]
            grp_mod = ["Casos","Honorarios","Pagos","CuotaLitis","Consultas","Actuaciones","Documentos","Usuarios","Colaboradores","Instancias","Contratos","Reportes"]
            grp_dash = ["Dashboard","Dash_Indicadores","Dash_Finanzas","Dash_Pendientes","Dash_Agenda","Dash_Reportes"]

            with st.form(f"perm_role_form_{rol_sel}"):
                st.caption(f"Configuras permisos para el rol: **{rol_sel}**")

                st.markdown("#### Acciones (CRUD)")
                c1,c2,c3,c4 = st.columns(4)
                val = {}
                for i, col in enumerate(grp_acc):
                    with [c1,c2,c3,c4][i]:
                        val[col] = "1" if st.checkbox(col, value=_perm_bool(base.get(col,"0")), key=f"pr_{rol_sel}_{col}") else "0"

                st.markdown("#### Módulos")
                cols = st.columns(3)
                for i, col in enumerate(grp_mod):
                    with cols[i % 3]:
                        val[col] = "1" if st.checkbox(col, value=_perm_bool(base.get(col,"0")), key=f"pr_{rol_sel}_{col}") else "0"

                st.markdown("#### Dashboard (secciones)")
                cols2 = st.columns(3)
                for i, col in enumerate(grp_dash):
                    with cols2[i % 3]:
                        val[col] = "1" if st.checkbox(col.replace("_"," "), value=_perm_bool(base.get(col,"0")), key=f"pr_{rol_sel}_{col}") else "0"

                save_btn = st.form_submit_button("💾 Guardar permisos del rol", disabled=not _is_admin())
                if save_btn:
                    # upsert
                    dfp = permisos.copy()
                    dfp = dfp[~((dfp["Scope"]=="ROLE") & (dfp["ScopeID"]==rol_sel))]
                    new_row = {"Scope":"ROLE","ScopeID":rol_sel}
                    for c in perm_cols:
                        new_row[c] = val.get(c, base.get(c,"0"))
                    dfp = pd.concat([dfp, pd.DataFrame([new_row])], ignore_index=True)
                    permisos = dfp
                    save_df("permisos", permisos)
                    st.success("✅ Permisos del rol guardados")
                    st.rerun()

        # ---- POR USUARIO ----
        with tab_u:
            users_list = usuarios["Usuario"].astype(str).tolist() if (usuarios is not None and not usuarios.empty and "Usuario" in usuarios.columns) else []
            if not users_list:
                st.info("No hay usuarios registrados.")
            else:
                user_sel = st.selectbox("Usuario", users_list, key="perm_user_sel")
                base_row = _perm_get("USER", user_sel)
                base = {c: "0" for c in perm_cols}
                if base_row is not None:
                    for c in perm_cols:
                        base[c] = str(base_row.get(c, "0"))

                # mostrar rol heredado
                rol_h = ""
                try:
                    rol_h = str(usuarios[usuarios["Usuario"].astype(str)==str(user_sel)].iloc[0].get("Rol",""))
                except Exception:
                    pass
                st.caption(f"Rol actual del usuario: **{rol_h}** (los permisos de USER sobrescriben al rol)")

                grp_acc = ["Ver","Agregar","Modificar","Borrar"]
                grp_mod = ["Casos","Honorarios","Pagos","CuotaLitis","Consultas","Actuaciones","Documentos","Usuarios","Colaboradores","Instancias","Contratos","Reportes"]
                grp_dash = ["Dashboard","Dash_Indicadores","Dash_Finanzas","Dash_Pendientes","Dash_Agenda","Dash_Reportes"]

                with st.form(f"perm_user_form_{user_sel}"):
                    st.markdown("#### Acciones (CRUD)")
                    c1,c2,c3,c4 = st.columns(4)
                    val = {}
                    for i, col in enumerate(grp_acc):
                        with [c1,c2,c3,c4][i]:
                            val[col] = "1" if st.checkbox(col, value=_perm_bool(base.get(col,"0")), key=f"pu_{user_sel}_{col}") else "0"
                    ...
                    for i, col in enumerate(grp_dash):
                        with cols2[i % 3]:
                            val[col] = "1" if st.checkbox(col.replace("_"," "), value=_perm_bool(base.get(col,"0")), key=f"pu_{user_sel}_{col}") else "0"

                    save_btn = st.form_submit_button("💾 Guardar permisos del usuario", disabled=not _is_admin())
                    if save_btn:
                        dfp = permisos.copy()
                        dfp = dfp[~((dfp["Scope"]=="USER") & (dfp["ScopeID"]==user_sel))]
                        new_row = {"Scope":"USER","ScopeID":user_sel}
                        for c in perm_cols:
                            new_row[c] = val.get(c, base.get(c,"0"))
                        dfp = pd.concat([dfp, pd.DataFrame([new_row])], ignore_index=True)
                        permisos = dfp
                        save_df("permisos", permisos)
                        st.success("✅ Permisos del usuario guardados")
                        st.rerun()

                # opción para borrar override del usuario
                st.divider()
                st.warning("⚠️ Quitar permisos específicos del usuario (volver a heredar del rol)")
                conf = st.text_input("Escribe ELIMINAR para confirmar", key=f"perm_user_del_{user_sel}")
                if st.button("🧹 Quitar override de este usuario", disabled=(not _is_admin()) or conf.strip().upper()!="ELIMINAR", key=f"perm_user_del_btn_{user_sel}"):
                    dfp = permisos.copy()
                    dfp = dfp[~((dfp["Scope"]=="USER") & (dfp["ScopeID"]==user_sel))]
                    permisos = dfp
                    save_df("permisos", permisos)
                    st.success("✅ Override eliminado")
                    st.rerun()

    # ==========================================================
    # TAB: GESTIÓN DE USUARIOS (TU CRUD, CONSERVADO)
    # ==========================================================
    with tab_usr:
        # =========================
        # ABOGADOS DISPONIBLES (garantiza ID + evita default ID 1)
        # =========================
        df_ab = load_df("abogados")
        if df_ab is None:
            df_ab = pd.DataFrame()
        ...
        if not df_ab.empty:
            ...
            bad = ids_num.isna() | (ids_num <= 0)
            for i in df_ab.index[bad].tolist():
                max_id += 1
        ...
        abogado_opts = []
        abogado_label = {}
        if not df_ab.empty and "ID" in df_ab.columns:
            tmp = df_ab.copy()
            tmp["ID"] = pd.to_numeric(tmp["ID"], errors="coerce").fillna(0).astype(int)
        ...
        if accion_u == "Nuevo":
            with st.form("usr_new"):
                ...
                if submit:
                    ...
                    if rol == "Abogado":
                        st.success(f"✅ Usuario creado y asociado a: {abogado_label.get(str(abogado_id),'')} (ID {abogado_id})")
                    else:
            ...
            if usuarios.empty:
                ...
                with st.form("usr_edit"):
                    ...
                    activo_new = st.selectbox(
                        "Activo",
                        ["1","0"],
                        index=0 if str(fila.get("Activo","1")) == "1" else 1,
                        key="usr_edit_activo"
                    )
                    ...
                    if rol_new == "Abogado":
                        st.markdown("### 👨‍⚖️ Abogado asociado (obligatorio)")
                        if abogado_id_new:
                            st.caption(f"Actualmente asociado a: {abogado_label.get(abogado_id_new,'')} (ID {abogado_id_new})")
                        if not abogado_opts:
                            st.error("❌ No hay abogados registrados para asociar. Registra abogados primero.")
                        abogado_id_new = st.selectbox(
                            "Selecciona abogado",
                            options=[""] + abogado_opts,
                            index=([""] + abogado_opts).index(abogado_id_new) if abogado_id_new in ([""] + abogado_opts) else 0,
                            format_func=_fmt_abogado,
